...
- **Driver 7 write paths are all exercised.** Verified against production on 2026-08-28: signed in, the plan loads, and rescheduling works, which covers the `find`/`sort`/`toArray` reads and the `moveSessions` transaction with its temp-date staging. Verified on 2026-08-29: a run logged on `/` drove `updateSession`'s `$set` and produced a new recap, so `upsertDailySummary`'s `replaceOne(upsert)` is exercised; an AI Rebuild preview-and-apply on `/plan` drove `rebuildFutureSessions`, so the guarded `bulkWrite` is exercised; and `savePushSubscription`'s `updateOne(upsert)` plus `deletePushSubscription`'s `deleteOne` were driven against a throwaway subscription document, inserted and deleted by endpoint, leaving the real subscription untouched. `upsertSessionExplanation` and the morning-note summary write share the recap's `replaceOne(upsert)` call shape, and `deletePushSubscriptionByEndpoint` shares the `deleteOne` shape, so those two are covered by inference rather than directly. Adding the tests listed above would close all of this permanently instead of by observation.
