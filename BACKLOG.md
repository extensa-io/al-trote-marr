...
- **One driver 7 write path is unexercised.** Verified against production on 2026-08-28: signed in, the plan loads, and rescheduling works, which covers the `find`/`sort`/`toArray` reads and the `moveSessions` transaction with its temp-date staging. Verified on 2026-08-29: a run logged on `/` drove `updateSession`'s `$set` and produced a new recap, so `upsertDailySummary`'s `replaceOne(upsert)` is exercised; and an AI Rebuild preview-and-apply on `/plan` drove `rebuildFutureSessions`, so the guarded `bulkWrite` is exercised. Every server action in both flows returned 200, the plan revalidated across the full date range, and the runtime logs carry no errors and no `recap failed` line. `upsertSessionExplanation` and the morning-note summary write share the recap's `replaceOne(upsert)` call shape, so the driver-level risk there is covered by inference rather than directly. Still not driven on driver 7: `deletePushSubscription`, which needs a real unsubscribe from Settings. Adding the tests listed above would close all of this permanently instead of by observation.
